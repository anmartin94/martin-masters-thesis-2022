  ...
  for val_set in val_paper_ids:
    for vs in val_set:
        if val_df is None:
            ...
            #print("LENGTH SO FAR", len(val_df.index))
...
def train_val_loop(train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks, model_name, epochs, batch_size, lr, deberta, device):
  ...
  if deberta:
    model = DebertaForSequenceClassification.from_pretrained(
        model_name, num_labels=2, output_attentions=False, 
  ...
  return f1_score(
      true_labels, predictions), precision_score(
          true_labels, predictions), recall_score(true_labels, predictions), loss_values
...
def main():
  ...
  if context == "1":
    ...
    context = False
    title = dataset_name
  #train_x, train_y = preprocess_data(train_file, val_file, context)
  datasets = preprocess_data(train_file, context)
  results = {}
  if cased == "cased":
    #models = ['bert-base-cased', 'allenai/scibert_scivocab_cased']
  ...
  for model_name in models:
    ...
    if model_name == 'microsoft/deberta-base':
      ...
      tokenizer = DebertaTokenizer.from_pretrained(
          'microsoft/deberta-base', do_lower_case=True)
    else:
      tokenizer = BertTokenizer.from_pretrained(
          model_name, do_lower_case=uncased)
    train_input_list, val_input_list, train_label_list, val_label_list, \
    train_mask_list, val_mask_list = \
    get_dataset_splits(datasets, tokenizer)
      ...
      for b_size in batch_sizes:
        ...
        for lr in learning_rates:
          ...
          if not os.path.isdir("output"):
            os.mkdir("output")
          png_name = "output/"+title+"-"+str(b_size)+"-"+str(lr)+"-"+datetime.datetime.now().strftime('%Y%m%d_%H:%M:%S_')+".png"
          for arr in losses:
            x = []
