...
Usage: 
The name of the training dataset filepath must be passed as a command line argument,
as well as the number of iterations per model per encoding type.
EXAMPLE: python3 thesis-ml-experiments.py datasets/reduced_train.csv 10 
...
def extract_datasets(train_name):
  ...
  Returns two sets of training features and labels: 
  one with positional context and the other without."""
